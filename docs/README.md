...
When you enter the pipeline directory you created, you'll notice several files generated by `nf-core`.
...
For now, we will focus on `main.nf`.
This is the core Nextflow script we will modify to include `Manta` in our custom pipeline.

The first few lines of `main.nf` should look similar to:
```
...
Locate the `IMPORT FUNCTIONS / MODULES /...` section.
Here, you'll add the `include` statement for the Manta module that you copied from Step 4.
Simply paste it directly below the existing `include` statements.
This statement will pull the module directly from the `modules` directory located in the root of your pipeline.
